...
class Traffic_data(object):
    ...
    def traffic_stream_data(ixia):
        ...
        tgen_data = ixia.session_info(mode='get_traffic_items')
        # GETTING STREAMS FROM SESSION INFO
        streams = tgen_data.traffic_config
        ...
        for item in stream_list:
            try:
                traffic_status = ixia.traffic_stats(mode='flow',
                                                    streams=item)
                traffic_data = traffic_status.flow[stream_num].rx.loss_percent
                # ADD STREAM NAME AND LOSS TO DICT
                traffic_dict[item] = traffic_data
            except:
                log.info('stream {} error handling caught'.format(item))
    ...
    def traffic_check(ixia, traffic_state, ):
        ...
        log.info('POLLING IXIA traffic STREAM STATUS...')
        # CLEAR IXIA COUNTERS
