...
app.config['CELERY_RESULT_BACKEND'] = 'mongodb://mastarke-lnx-v2:27017/meritDB'
...
celery = Celery('task',broker='mongodb://mastarke-lnx-v2:27017/jobs')
...
def packages():
    ...
    if request.method == 'POST':
        ...
        app.logger.info('Matthew ping_output is {}'.format(ping_output))
        ping_success_num = re.search(r'(\d+)% +packet loss', ping_output)
        ...
        if ping_result == True:
            ...
            app.logger.info('matthew command_output is {}'.format(command_output))

            dir_found = re.search(r'\/\w+\/\w+\/\w+\/.*', command_output)
            app.logger.info('matthew dir_found is {}'.format(dir_found))
            path = re.sub(r':', '', dir_found.group(0))
            app.logger.info('matthew path is {}'.format(path))
            image_repo = path
...
if __name__ == '__main__':
    linux_hostname = os.popen('hostname --short').read()
    linux_hostname = linux_hostname.rstrip("\n\r")
    app.secret_key = 'super secret key'
    app.run(debug=True, port=1111, host=linux_hostname)
